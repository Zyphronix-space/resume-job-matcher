...
def get_current_recruiter(user: db_models.User = Depends(get_current_user)) -> db_models.User:
    if user.role != "recruiter":
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Recruiter access required")
    return user


def get_current_candidate(user: db_models.User = Depends(get_current_user)) -> db_models.User:
